...
class DeclarationGenerator:
    ...
    def calculate(self):
        ...
        for date in self.ens_data.get('insurance_paid_dates', []):
            if date and date.year == 2025:
                paid_in_2025 += self.ens_data.get('insurance_paid', 0)
                break
    ...
    def generate_excel(self, filepath):
        ...
        quarter_sum = {
            1: self.quarterly_income[1],
            2: self.quarterly_income[1] + self.quarterly_income[2],
            3: self.quarterly_income[1] + self.quarterly_income[2] + self.quarterly_income[3],
            4: self.total_income
        }
        
        rows_income = [
            ("Доход за 1 квартал", "110", quarter_sum[1]),
            ("Доход за полугодие", "111", quarter_sum[2]),
            ("Доход за 9 месяцев", "112", quarter_sum[3]),
            ("Доход за год", "113", quarter_sum[4]),
            ("Налоговая ставка (%)", "120", self.tax_rate),
            ("Сумма налога за 1 квартал", "130", quarter_sum[1] * self.tax_rate / 100),
            ("Сумма налога за полугодие", "131", quarter_sum[2] * self.tax_rate / 100),
            ("Сумма налога за 9 месяцев", "132", quarter_sum[3] * self.tax_rate / 100),
        ...
        rows_tax = [
            ...
            ("Налог к уплате за год", "100", round(self.tax_payable, 2)),
        ]
    ...
    def generate_xml(self, filepath):
        """Генерация XML-файла декларации"""
        self.calculate()
...
<Файл xmlns="urn:ФНС-СХД-Декл-УСН-2025-1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <Показатели>
        ...
        <Раздел2_1_1>
            <СумДоход110>{int(quarter_sum[1])}</СумДоход110>
            <СумДоход111>{int(quarter_sum[2])}</СумДоход111>
            <СумДоход112>{int(quarter_sum[3])}</СумДоход112>
            <СумДоход113>{int(quarter_sum[4])}</СумДоход113>
            <НалСтавка120>{self.tax_rate}</НалСтавка120>
            <СумИсчисНал130>{int(quarter_sum[1] * self.tax_rate / 100)}</СумИсчисНал130>
            <СумИсчисНал131>{int(quarter_sum[2] * self.tax_rate / 100)}</СумИсчисНал131>
            <СумИсчисНал132>{int(quarter_sum[3] * self.tax_rate / 100)}</СумИсчисНал132>
